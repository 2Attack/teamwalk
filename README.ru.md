...
Кнопка форкает репозиторий, спрашивает переменные (локаль, опциональный Telegram-бот, секрет крона) и подключает **Neon Postgres** из Marketplace — `DATABASE_URL` подставляется автоматически, `buildCommand` прогоняет миграции. LLM-подсказки работают из коробки: на Vercel AI SDK аутентифицируется автоматическим `VERCEL_OIDC_TOKEN`.
