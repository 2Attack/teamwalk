...
/** Office timezone. Streak and week day boundaries are computed in it. */
...
/** Treadmill list position bounds; the column is a smallint. */
 ...
 * LLM — same margin reasoning as HINTS_LLM_TIMEOUT_MS.
 */
export const ROUTE_LLM_TIMEOUT_MS = 45_000;

/** Default speed for a new participant. */
export const DEFAULT_SPEED_KMH = 4;

/** A walk active longer than this is forgotten and auto-closed. */
export const STALE_WALK_HOURS = 8;

/** Window for deleting one's own record after finishing. */
export const DELETE_WINDOW_MINUTES = 15;

/** Soft warnings in the finish dialog. */
...
/** Streak freezes per calendar month. */
...
/** Below this count after filtering the pool is topped up with static phrases. */
export const HINTS_MIN_AFTER_FILTER = 6;
/*
  The spec says 15 s, but on a live key Gemini Flash does not always fit:
...
/** A participant younger than this many days is not teased. */
export const HINTS_NEWCOMER_DAYS = 3;

/**
 * Telegram notifications. Without a token the subsystem is off
...
/** TTL of the one-time link token. */
export const TG_LINK_TOKEN_TTL_MINUTES = 15;

/** "Time to stretch" reminders: all intervals are in workdays. */
...
/**
 * The "treadmill freed" window is wider: the narrow reminder
