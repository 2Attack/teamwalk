...
El botón hace fork del repo, pide las variables (idioma, bot de Telegram opcional, secreto del cron) y aprovisiona un **Neon Postgres** del Marketplace — `DATABASE_URL` se inyecta automáticamente y `buildCommand` ejecuta las migraciones. Las pistas con LLM funcionan de serie: en Vercel el AI SDK se autentica con el `VERCEL_OIDC_TOKEN` automático.
