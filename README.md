...
| `NEXT_PUBLIC_LOCALE` | нет | Язык всего продукта: `en` (по умолчанию), `ru`, `es`. Действует на UI, ошибки API, каталог хинтов, промпты LLM и тексты Telegram-бота. Инлайнится в клиентский бандл при сборке — смена значения требует redeploy |
